...
export default class WordHandler {
  ...
  constructor(starterWord?: string, length = 5) {
    this._wordList = ["test"]; // fs.readFileSync(wordListPath, 'utf8').split('\n').filter((word: string) => word.length === length);
    if (starterWord) {this._targetWord = starterWord;} else {this.randomizer();}
    this._targetWord = this._targetWord.toLocaleLowerCase();
    this.fillMap(this._targetWord);
  ...
  /**
     ...
     */
  fillMap(word: string): void {
    for (let i = 0; i < word.length; i += 1) {
      const currentChar = word[i];
      if (this._wordMap.has(currentChar)) {
        const currentMapValue = this._wordMap.get(currentChar);
        this._wordMap.set(currentChar, (currentMapValue || 0) + 1);
      } else {
        this._wordMap.set(currentChar, 1);
      }
    }
  }
